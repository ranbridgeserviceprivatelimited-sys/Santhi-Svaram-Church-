    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel p-8 rounded-3xl border border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Review worker leave applications, approve/reject requests, and dispatch worker notifications.</p>
        </div>
                ...
                filteredLeaves.map((l) => (
                  <tr key={l.id} className="hover:bg-slate-900/60 transition-colors">
                    <td className="p-4">
                      ...
                      <span className="text-[10px] text-amber-400">{l.department}</span>
                    </td>

                    <td className="p-4">
                      <span className="px-2.5 py-0.5 rounded-full bg-slate-950 text-slate-300 border border-slate-800 font-semibold text-[11px]">
                        {l.leaveType}
                      </span>
                    </td>

                    <td className="p-4 space-y-0.5">
                      <div className="font-semibold text-slate-200">{l.fromDate} to {l.toDate}</div>
                      <div className="text-amber-400 font-bold text-[10px]">{l.days} Days</div>
                    </td>

                    <td className="p-4 text-slate-300 max-w-xs truncate" title={l.reason}>
                      "{l.reason}"
                    </td>
                    ...
                    <td className="p-4 text-right">
                      {l.status === 'Pending' ? (
                        <button
                          ...
                          className="px-3.5 py-1.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-xl text-xs shadow-md transition-all"
                        >
                          Review Request
                        </button>
                      ) : (
