      ...
      <div className="relative border-l-2 border-amber-300 ml-4 sm:ml-32 space-y-12 pl-6 sm:pl-10">
        {historyMilestones.map((item, idx) => (
          <div key={idx} className="relative group">
            {/* Year Node Badge */}
            <div className="absolute -left-[31px] sm:-left-[55px] top-0 flex items-center justify-center w-12 h-12 rounded-2xl bg-amber-600 text-white font-serif-spiritual font-bold text-lg shadow-lg group-hover:scale-110 group-hover:bg-amber-700 transition-all">
              {item.year.slice(2)}'
            </div>

            {/* Timeline Content Card */}
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 space-y-4">
      ...
      <div className="bg-slate-900 text-white rounded-3xl p-8 sm:p-12 shadow-2xl space-y-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
          <div>
            ...
            <h2 className="font-serif-spiritual text-3xl font-bold text-white mt-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredArchive.map((arc) => (
            <div key={arc.id} className="bg-slate-800/80 border border-slate-700/60 rounded-2xl p-6 hover:border-amber-500/50 transition-all flex flex-col justify-between space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs text-amber-400 font-semibold">
                  ...
                  <span className="text-slate-400 font-mono">{arc.type}</span>
                </div>
                <h4 className="font-serif-spiritual text-xl font-bold text-slate-100">
                  {arc.title}
                </h4>
              ...
              <div className="pt-2 flex items-center justify-between border-t border-slate-700/40 text-xs">
                <span className="text-slate-400">Archival Clearance: Public</span>
                <button
                  onClick={() => alert(`Opening digital archive item: ${arc.title}`)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-500 text-slate-950 font-bold hover:bg-amber-400 transition-colors"
                >
